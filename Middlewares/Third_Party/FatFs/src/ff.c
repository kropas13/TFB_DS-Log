...
#else
			// DANI if (IsLower(w)) w -= 0x20;			/* To upper ASCII characters */
#ifdef _EXCVT
...
/*-----------------------------------------------------------------------*/
#define N_ROOTDIR	16 // DANI 512		/* Number of root directory entries for FAT12/16 */
...
)
{
	...
	if (_MULTI_PARTITION && part) {
		...
		if (disk_ioctl(pdrv, GET_SECTOR_COUNT, &n_vol) != RES_OK || n_vol < 8) // DANI < 128
			return FR_DISK_ERR;
    ...
    if (fmt == FS_FAT12)
      mem_cpy(tbl + BS_VolLab, VOL_LABEL_NAME "FAT12   ", 19);	/* Volume label, FAT signature */
    else
