...
/*!
 * Global MAC layer Configurations as added by comtac
 */
typedef struct
{
    uint8_t MinTxDatarate;
    uint8_t MaxTxDatarate;
    uint8_t DefaultTxDatarate;
    uint8_t Rx2DefaultDatarate;
}LoRaMacCfg_t;
...
/*!
 * comtac ADD : For tests to meassure the Rx1 window lenght (active when window is open)
 *
 * @retval None
 */  
    void ( *TestSetRx1WindowsState ) ( bool bState);
/*!
 * comtac ADD : For tests to meassure the Rx2 window lenght (active when window is open)
 *
 * @retval None
 */      
    void ( *TestSetRx2WindowsState ) ( bool bState);  
 ...
 *
 * \param   [IN] Configurations - Pointer to a structure defining the LoRaMAC
 *                        configs as defined by comtac. Refer to \ref LoRaMacCfg_t.
 * 
